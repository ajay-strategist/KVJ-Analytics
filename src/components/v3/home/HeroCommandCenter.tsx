 ...
 * Replaced GSAP with hardware-accelerated native CSS animations and passive scroll listeners.
 */
export function HeroCommandCenter({
    ...
    const onScroll = () => {
      if (!raf) {
        raf = requestAnimationFrame(() => {
          ...
          if (headline) {
            const opacity = Math.max(0.35, 1 - sy / 400);
            const blur = Math.min(3, sy / 100);
            const y = sy * -0.15;
            headline.style.opacity = String(opacity);
            headline.style.filter = sy > 0 ? `blur(${blur}px)` : "none";
            headline.style.transform = `translate3d(0, ${y}px, 0)`;
          }
          if (scrollInd) {
            scrollInd.style.opacity = String(Math.max(0, 1 - sy / 150));
            scrollInd.style.transform = `translate3d(-50%, ${sy * 0.15}px, 0)`;
          }
          raf = 0;
        });
      }
  ...
  return (
    ...
    >
      ...
      <div className="beam absolute top-[8%] right-[2%] h-[44rem] w-[26rem] bg-emerald-500/10" />
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1340px] px-5 sm:px-6 lg:px-8 py-28 lg:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-12 lg:gap-8 items-center">
          ...
          <div className="max-w-xl">
            <Reveal variant="up" delay={50}>
              <h1
                ...
                style={{ perspective: "1000px", transition: "opacity 0.6s ease, transform 0.6s ease" }}
              >
                {headline.replace(/\.$/, "")}
              </h1>
            </Reveal>
          ...
          <div className="relative hidden lg:flex items-center justify-center h-[600px] w-full">
            <Reveal variant="scale" delay={100} className="w-full h-full">
              <div
                ...
                >
                  ...
                  <div className="flex gap-2 items-center" style={{ transform: "translateZ(15px)" }}>
                    ...
                    <svg className="w-12 h-12" viewBox="0 0 36 36">
                      <g
                        style={{
                          transform: mounted ? "rotate(0deg) scale(1)" : "rotate(-90deg) scale(0.8)",
                          opacity: mounted ? 1 : 0,
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="text-[14px] font-bold text-ink tabular-nums">$45,231</div>
                    ...
                    <div className="relative w-12 h-12 flex items-center justify-center shrink-0">
                      <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                        ...
                        <circle
                          ...
                          strokeWidth="3.5"
                          strokeDasharray="100"
                          strokeDashoffset={mounted ? 25 : 100}
                          strokeLinecap="round"
                          style={{
