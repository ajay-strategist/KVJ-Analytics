...
export default async function CourseDetailPage({
  ...
  // Price: tolerate both schemas. The target schema's `fee_inr` defaults to 0, so a 0 there must
  // fall through to the legacy `price_inr` (10000) rather than winning — hence `||`, not `??`.
  const fee_inr = Number(course?.fee_inr) || Number(course?.price_inr) || (fallback?.fee_inr ?? 0);
  ...
  const banner_url = course?.banner_url || course?.thumbnail_url || fallback?.banner_url || "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800";
