...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1b. Bind enrollment to the AUTHORITATIVE order record (never trust the client
    //     body for course/user/amount — the signature only covers order|payment id).
    const { data: orderRow } = await supabaseAdmin
      .from("orders")
      .select("user_id, course_slug, amount, status")
    ...
    // Idempotency: already processed → succeed without double-writing.
    if (orderRow.status === "paid") {
      return NextResponse.json({ success: true, alreadyProcessed: true });
    }

    const finalUserId = orderRow.user_id;
    const finalCourseSlug = orderRow.course_slug;
    const finalAmount = Number(orderRow.amount ?? amount ?? 0);
    ...
    const { data: authUserData, error: authUserError } = await supabaseAdmin.auth.admin.getUserById(finalUserId);
    ...
    // 5. Create course enrollment (using the trusted order values)
