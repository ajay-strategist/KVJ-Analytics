...
`.trim();
  ...
  const handleCopyAiPromptTemplate = () => {
    const selectedCourseObj = courses.find((c) => c.id === newCourseId);
    const courseIdVal = selectedCourseObj?.id || "COURSE_ID";
    const courseTitleVal = selectedCourseObj?.title || "Artificial Intelligence";
    const campaignIdVal = getAutoGeneratedIdPreview();
    const campaignNameVal = newCampName || "New Campaign";
    const trainingModeVal = newTrainingMode || "online";
