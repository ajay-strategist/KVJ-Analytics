...
`.trim();
  ...
  const handleCopyAiPromptTemplate = () => {
    const selectedCourseObj = courses.find((c) => c.id === newCourseId);
    const promptText = `
You are generating a registration form for KVJ Analytics.

The form will be embedded inside the KVJ Analytics campaign registration system.

The HTML must preserve the KVJ Analytics integration contract.

Do not remove or rename the required form attributes, hidden fields, field names, campaign context, course context, or submission endpoint.

The form must submit registration data to the KVJ Analytics registration endpoint.

Campaign Name:
${newCampName || "New Campaign"}

Campaign ID:
${getAutoGeneratedIdPreview()}

Course ID:
${selectedCourseObj?.id || "SELECTED_COURSE_ID"}

Course:
${selectedCourseObj?.title || "Artificial Intelligence"}

Training Mode:
${newTrainingMode || "online"}

REQUIRED FORM CONTRACT:
<form data-kvj-form="registration" data-kvj-endpoint="/api/register" data-kvj-course="${selectedCourseObj?.id || "COURSE_ID"}">
  <input type="hidden" name="campaign_id" value="${getAutoGeneratedIdPreview()}">
  <input type="hidden" name="course_id" value="${selectedCourseObj?.id || "COURSE_ID"}">
  <input type="hidden" name="training_mode" value="${newTrainingMode || "online"}">
