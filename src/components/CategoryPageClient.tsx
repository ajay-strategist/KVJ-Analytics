...
export function CategoryPageClient({ categorySlug, categoryName, categoryDesc, courses }: CategoryPageClientProps) {
  ...
  useEffect(() => {
    const checkAuthAndEnrollments = async () => {
      ...
      if (session?.user) {
        ...
        if (enrolls) {
          setEnrolledSlugs(new Set(enrolls.map((e: any) => e.course_slug)));
        }
  ...
  return (
    <div className="w-full bg-base text-zinc-200 min-h-screen pt-28 pb-24 relative overflow-hidden text-left">
      {/* Background spotlights */}
      <div className="absolute top-[20%] left-[-10%] w-[500px] h-[500px] bg-[#43F5FF]/5 rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute bottom-[20%] right-[-10%] w-[600px] h-[600px] bg-[#3A7BFF]/5 rounded-full blur-[160px] pointer-events-none" />
      ...
      <Container>
        ...
        <div className="max-w-4xl mb-16 text-left">
          <div className="flex items-center gap-2 text-sm text-zinc-400 font-light mb-4">
            ...
            <span>/</span>
            <span className="text-[#43F5FF] font-semibold">{categoryName}</span>
          </div>

          <h1 className="font-display font-bold text-4xl sm:text-5xl text-white tracking-tight leading-none">
            {categoryName} <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#43F5FF] via-[#3A7BFF] to-[#43F5FF] bg-[size:200%_auto]">Training</span>
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 max-w-6xl mx-auto">
          ...
          <div className="lg:col-span-7 space-y-10">
            ...
            {/* Enter unlock code */}
            <Card hoverLift={false} className="border border-[#43F5FF]/25 bg-[#43F5FF]/5 p-6 rounded-2xl relative overflow-hidden">
              ...
              {unlockError && (
                <div className="bg-rose-500/5 border border-rose-500/15 p-3 rounded-xl flex items-start space-x-2 text-rose-455 mt-4">
              ...
              <form onSubmit={handleUnlockSubmit} className="flex gap-2 mt-4">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl border border-line text-sm bg-[#0A0D13] text-white placeholder-zinc-500 focus:outline-none focus:border-[#43F5FF]/40 text-center font-mono tracking-wider font-bold"
                />
                <Button
                  type="submit"
                  disabled={unlockLoading}
                  className="py-2.5 px-5 bg-gradient-to-r from-[#43F5FF] to-[#3A7BFF] text-black font-extrabold text-xs border-none flex items-center justify-center shrink-0"
                >
              ...
              {!user && (
                <p className="text-[10px] text-zinc-500 mt-2 text-center">
                  * You must be signed in to student account to redeem.{" "}
                  <Link href={`/signin?redirect=/training/${categorySlug}`} className="text-[#43F5FF] font-bold underline">Log In</Link>
                </p>
              )}
            </Card>
                ...
                <div className="space-y-4">
                  {courses.map((course) => {
                    ...
                    return (
                      <div
                        key={course.id}
                        className="bg-[#0E1117]/72 border border-line p-5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-6 hover:border-[#3A7BFF]/30 transition-all duration-300 relative group overflow-hidden"
                      >
                        <div className="flex gap-4 items-center">
                          <div className="w-14 h-14 rounded-xl overflow-hidden shrink-0 border border-line bg-[#0A0D13] hidden sm:block">
                            {course.banner_url ? (
                              <img src={course.banner_url} alt={course.title} className="w-full h-full object-cover" />
                            ) : (
                              <div className="w-full h-full bg-zinc-900 flex items-center justify-center text-[#43F5FF]">
                                <Laptop className="w-6 h-6" />
                              </div>
                            )}
                          </div>
                          <div>
                            <h4 className="text-md font-bold text-white group-hover:text-[#43F5FF] transition-colors leading-tight">
                              {course.title}
                            </h4>
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          {isUnlocked ? (
                            <Link href={`/training/${course.slug}/learn`}>
                              <Button className="py-2 px-4 bg-[#43F5FF] text-black font-bold text-xs border-none flex items-center gap-1">
                                Launch Program
                              </Button>
                            </Link>
                          ) : (
                            <Link href={`/training/${course.slug}`}>
                              <Button variant="secondary" className="py-2 px-4 bg-[#0A0D13]/50 border-line text-zinc-300 text-xs flex items-center gap-1.5">
          ...
          <div className="lg:col-span-5 text-left">
            <div className="lg:sticky lg:top-28">
              {inquirySuccess ? (
                ...
                <Card hoverLift={false} className="bg-[#0E1117]/72 border border-line p-8 rounded-3xl backdrop-blur-xl">
                  ...
                  <form onSubmit={handleInquirySubmit} className="space-y-4">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-12 pr-4 py-3 rounded-xl border border-line text-sm bg-[#0A0D13] text-white placeholder-zinc-500 focus:outline-none focus:border-[#43F5FF]/40"
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full px-4 py-3 rounded-xl border border-line text-sm bg-[#0A0D13] text-white placeholder-zinc-500 focus:outline-none focus:border-[#43F5FF]/40"
                      />
                    </div>
                    ...
                    <Button
                      ...
                      className="w-full py-3.5 bg-gradient-to-r from-[#43F5FF] to-[#3A7BFF] text-black font-extrabold flex items-center justify-center gap-1.5 border-none"
                    >
                      {inquiryLoading ? (
