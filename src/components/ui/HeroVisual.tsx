  ...
  return (
    <div className="relative w-full max-w-none flex items-center justify-center py-6 select-none font-body">
      ...
      <div className="relative w-full p-5 rounded-[24px] bg-[#0B2A22]/72 backdrop-blur-[24px] border border-line shadow-soft flex flex-col gap-5 overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-line pb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono font-bold tracking-[0.2em] text-slate">
              [ REAL-TIME METRICS BROADCAST ]
            </span>
          </div>
          <div className="text-[8px] font-mono text-slate bg-white/5 border border-line px-2 py-0.5 rounded uppercase tracking-wider">
            STABLE // LATENCY 12ms
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-stretch">
          ...
          <div className="md:col-span-8 rounded-xl border border-line bg-white/[0.01] p-4 flex flex-col justify-between group hover:border-brand/20 transition-all duration-300">
            <div className="flex justify-between items-center mb-3">
              <span className="text-[10px] font-mono uppercase tracking-widest text-slate font-bold">
            ...
            <div className="relative w-full h-[125px] overflow-hidden">
              <svg viewBox={`0 0 ${svgWidth} ${totalSvgHeight}`} width="100%" height="100%" preserveAspectRatio="none" className="overflow-hidden">
                ...
                {/* Grid Lines */}
                <line x1="0" y1={svgHeight * 0.25} x2={svgWidth} y2={svgHeight * 0.25} stroke="rgba(60,255,255,0.05)" strokeWidth="1" />
                <line x1="0" y1={svgHeight * 0.5} x2={svgWidth} y2={svgHeight * 0.5} stroke="rgba(60,255,255,0.05)" strokeWidth="1" />
                <line x1="0" y1={svgHeight * 0.75} x2={svgWidth} y2={svgHeight * 0.75} stroke="rgba(60,255,255,0.05)" strokeWidth="1" />
                  ...
                  {points.map((p, idx) => {
                    ...
                    return (
                      <text
                        ...
                        y={svgHeight + 15}
                        textAnchor="middle"
                        fill={idx === lastIdx ? "#ffffff" : "#A7B1C4"}
          ...
          <div className="md:col-span-4 flex flex-col sm:flex-row md:flex-col gap-4">
            ...
            <div className="flex-1 rounded-xl border border-line bg-white/[0.01] p-4 flex flex-col items-center justify-center relative overflow-hidden group hover:border-brand/20 transition-all duration-300">
              <span className="text-[9px] font-mono uppercase tracking-widest text-slate mb-3 block text-center font-bold">
                SYSTEM EFFICIENCY
              </span>
              <div className="relative w-20 h-20 flex items-center justify-center">
                ...
                <div className="absolute flex flex-col items-center justify-center">
                  ...
                  <span className="text-[6px] font-mono text-slate mt-1 uppercase tracking-widest">
                    OPTIMIZED
                  </span>
                </div>
              </div>
            ...
            <div className="flex-1 rounded-xl border border-line bg-white/[0.01] p-4 flex flex-col justify-between group hover:border-brand/20 transition-all duration-300">
              <span className="text-[9px] font-mono uppercase tracking-widest text-slate mb-2 block text-center font-bold">
                LOAD DISTRIBUTION
              </span>
                ...
                {barValues.map((val, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex flex-col items-center flex-1">
                      ...
                      <span className="text-[7.5px] font-mono text-slate font-bold mt-1.5">{labels[idx]}</span>
                    </div>
                  );
                })}
              </div>
        ...
        <div className="rounded-xl border border-line bg-[#050608]/90 p-4 font-mono text-[9px] relative overflow-hidden group">
          <div className="absolute top-3.5 right-4 text-slate flex items-center gap-1.5">
          ...
          <div className="space-y-2 max-h-[85px] overflow-hidden">
            {logs.map((log) => (
              <div key={log.id} className="flex items-start gap-2.5 text-left transition-all duration-300 ease-out">
                <span className="text-brand/90 font-bold">[{log.date}]</span>
                <span className="text-slate font-bold">&gt;</span>
                <span className="text-slate flex-1 break-all font-medium">{log.text}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="absolute top-[2%] right-[-15px] rounded-2xl border border-brand/35 bg-[#0B2A22]/95 p-3.5 shadow-soft max-w-[145px] z-10 transition-transform duration-300 hover:scale-105 backdrop-blur-xl">
        ...
        <p className="text-[8px] text-slate leading-normal font-mono font-medium text-left">
