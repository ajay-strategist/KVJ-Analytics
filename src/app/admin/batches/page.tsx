...
export default function AdminBatchesPage() {
  ...
  const [studentMode, setStudentMode] = useState<"upload" | "form">("upload");
  ...
  const handleAddSingleStudent = async (e: React.FormEvent) => {
    ...
    const trimmedName = singleStudent.name.trim();
    const trimmedEmail = singleStudent.email.trim();
    const trimmedPhone = singleStudent.phone.trim();
    const trimmedId = singleStudent.student_id.trim();
    const trimmedDept = singleStudent.department.trim();

    if (!trimmedName) {
      setSingleStudentMsg({ type: "error", text: "Please enter the student's name." });
      return;
    }
    if (!trimmedPhone && !trimmedEmail) {
      setSingleStudentMsg({ type: "error", text: "Please provide either a phone number or an email." });
      return;
    }
    ...
    try {
      ...
      if (data.skipped > 0) {
        setSingleStudentMsg({
          type: "error",
          text: "A student with this phone or email already exists in this batch roster.",
        });
      } else {
        setSingleStudentMsg({
          type: "success",
          text: `Added "${trimmedName}" to the roster successfully!`,
        });
        setSingleStudent({
          name: "",
          email: "",
        ...
        fetchRoster(activeBatchForStudents.id);
      }
    } catch (err: any) {
      setSingleStudentMsg({
        type: "error",
                          ...
                          <div className="flex items-center justify-center gap-1">
                            <button
                              onClick={() => {
                                ...
                                setPreviewInvalidCount(0);
                                setImportResult(null);
                                setStudentMode("upload");
                                setSingleStudent({ name: "", email: "", phone: "", student_id: "", department: "" });
                                setSingleStudentMsg(null);
                              }}
        ...
        <div className="fixed inset-0 z-[150] grid place-items-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-6xl max-h-[90vh] flex flex-col rounded-2xl border border-slate-200 bg-white shadow-xl overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden grid grid-cols-1 lg:grid-cols-5 divide-y lg:divide-y-0 lg:divide-x divide-slate-100">
              ...
              {/* Left Column: Import / Upload or Manual Add */}
              <div className="lg:col-span-2 p-6 overflow-y-auto space-y-5 flex flex-col justify-start">
                ...
                {studentMode === "upload" ? (
                  ...
                  <form onSubmit={handleAddSingleStudent} className="space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        Manually authorize an individual student to join this batch.
                      </p>
                    </div>
                    ...
                    {singleStudentMsg && (
                      ...
                      >
                        ...
                        <span>{singleStudentMsg.text}</span>
                      </div>
                    )}
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full text-xs px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand bg-white"
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-bold text-slate-700 mb-1">
                            Phone Number <span className="text-slate-400 font-normal">(Primary)</span>
                          </label>
                          <input
                            type="tel"
                            value={singleStudent.phone}
                            onChange={(e) => setSingleStudent({ ...singleStudent, phone: e.target.value })}
                            placeholder="e.g. 9876543210"
                            className="w-full text-xs px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand bg-white font-mono"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setSingleStudent({ ...singleStudent, student_id: e.target.value })}
                    ...
                    <div className="pt-2 flex items-center gap-2">
                      <Button
                        ...
                        className="flex-1 justify-center py-2.5 bg-brand text-white hover:bg-brand/90 font-bold text-xs rounded-xl shadow-sm cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="py-2.5 px-3 border border-slate-200 hover:bg-slate-50 text-slate-500 rounded-xl text-xs font-medium cursor-pointer transition-colors"
                        title="Clear form"
                      >
                        Clear
                      </button>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-xs font-bold text-brand bg-brand/10 hover:bg-brand/15 px-2.5 py-1 rounded-lg transition-colors cursor-pointer"
                      title="Add a student manually"
                    >
